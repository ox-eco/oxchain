...
namespace OX.Compiler.MSIL
{
    ...
    {
        private void _ConvertStLoc(ILMethod method, OpCode src, OXMethod to, int pos)
        {

            //get array
            //_Convert1by1(OX.VM.OpCode.FROMALTSTACK, src, to);
            //_Convert1by1(OX.VM.OpCode.DUP, null, to);
            //_Convert1by1(OX.VM.OpCode.TOALTSTACK, null, to);
            _Convert1by1(OX.VM.OpCode.DUPFROMALTSTACK, src, to);
            ...
            _Convert1by1(OX.VM.OpCode.ROLL, null, to);

            _Convert1by1(OX.VM.OpCode.SETITEM, null, to);


            //_Convert1by1(OX.VM.OpCode.CLONESTRUCTONLY, src, to);
            ////push d
            //var c = _Convert1by1(OX.VM.OpCode.DEPTH, null, to);
            //if (c.debugcode == null)
            //{
            ...
            ////push olddepth
            //_Convert1by1(OX.VM.OpCode.FROMALTSTACK, null, to);
            //_Convert1by1(OX.VM.OpCode.DUP, null, to);
            //_Convert1by1(OX.VM.OpCode.TOALTSTACK, null, to);
            ////(d-n-1)-olddepth
            //_Convert1by1(OX.VM.OpCode.SUB, null, to);
            ...
            ////swap d-n-1 and top
            //_Convert1by1(OX.VM.OpCode.XSWAP, null, to);
            ////drop top
            //_Convert1by1(OX.VM.OpCode.DROP, null, to);

        }
        private void _ConvertLdLoc(ILMethod method, OpCode src, OXMethod to, int pos)
        {
            //get array
            //_Convert1by1(OX.VM.OpCode.FROMALTSTACK, src, to);
            //_Convert1by1(OX.VM.OpCode.DUP, null, to);
            ...
            //get i
            _ConvertPush(pos + method.paramtypes.Count, null, to);//翻转取参数顺序
            _Convert1by1(OX.VM.OpCode.PICKITEM, null, to);


        }
        private void _ConvertLdLocA(ILMethod method, OpCode src, OXMethod to, int pos)
        ...
        private void _ConvertCastclass(ILMethod method, OpCode src, OXMethod to)
        {
        ...
        private void _ConvertLdArg(ILMethod method, OpCode src, OXMethod to, int pos)
        {
            try
            {
            ...
            ////push d
            //var c = _Convert1by1(OX.VM.OpCode.DEPTH, src, to);
            //if (c.debugcode == null)
            //{
            ...
            ////push n
            //_ConvertPush(pos, null, to);//翻转取参数顺序
            ////_Convert1by1(OX.VM.OpCode.PUSHDATA1, null, to, int2Pushdata1bytes(to.paramtypes.Count - 1 - pos));
            ////d+n
            //_Convert1by1(OX.VM.OpCode.ADD, null, to);
        ...
        private void _ConvertStArg(OpCode src, OXMethod to, int pos)
        {
            //get array
            _Convert1by1(OX.VM.OpCode.DUPFROMALTSTACK, src, to);
            //set i
        ...
        */
        ...
        public bool IsMixAttribute(Mono.Cecil.MethodDefinition defs, out OX.VM.OpCode[] opcodes, out string[] opdata)
        {
            // ============================================
            ...
            opcodes = new OX.VM.OpCode[count_attrs];
            ...
            foreach (var attr in defs.CustomAttributes)
            {
                if (attr.AttributeType.Name == "OpCodeAttribute")
                {
                    opcodes[i] = (OX.VM.OpCode)attr.ConstructorArguments[0].Value;
                ...
                else if (attr.AttributeType.Name == "SyscallAttribute")
                {
                    ...
                    opcodes[i] = OX.VM.OpCode.SYSCALL;
                    opdata[i] = val;
            ...
            else
            {
                ...
                throw new Exception("oxmachine Cannot mix OpCode/Syscall/Script attributes with others!");
            }
        }

        /*
                public bool IsOpCall(Mono.Cecil.MethodDefinition defs, out OX.VM.OpCode[] opcodes)
                {
                    opcodes = null;
                    if (defs == null)
                    {
                    ...
                    foreach (var attr in defs.CustomAttributes)
                    {
                        if (attr.AttributeType.Name == "OpCodeAttribute")
                        {
                            ...
                            opcodes = new OX.VM.OpCode[val.Length];
                            for (var j = 0; j < val.Length; j++)
                            {
                                opcodes[j] = ((OX.VM.OpCode)(byte)val[j].Value);
                            }
        ...
        */
        ...
        public bool IsNotifyCall(Mono.Cecil.MethodDefinition defs, Mono.Cecil.MethodReference refs, OXMethod to, out string name)
        {
        ...
        private int _ConvertCall(OpCode src, OXMethod to)
        {
            ...
            int callpcount = 0;
            byte[] callhash = null;
            //OX.VM.OpCode callcode = OX.VM.OpCode.NOP;
            OX.VM.OpCode[] callcodes = null;
            ...
            //if (System.Enum.TryParse<OX.VM.OpCode>(callname, out callcode))
            //{
            //    calltype = 2;
            //}
            //else
            ...
            else if (IsMixAttribute(defs, out callcodes, out calldata))
            {
                ...
                if (callcodes.Length == 1 && callcodes[0] != OX.VM.OpCode.SYSCALL)
                {
                    calltype = 2;
                }
            }
            ...
            {//maybe a syscall // or other
                ...
                else if (src.tokenMethod.Contains("::op_Equality(") || src.tokenMethod.Contains("::Equals("))
                {
                    ...
                    {
                        _Convert1by1(OX.VM.OpCode.EQUAL, src, to);
                ...
                else if (src.tokenMethod.Contains("::op_Inequality("))
                {
                    ...
                    if (_ref.DeclaringType.FullName == "System.Boolean"
                        ...
                        || _ref.DeclaringType.FullName == "System.Numerics.BigInteger")
                    {
                        _Convert1by1(OX.VM.OpCode.NUMNOTEQUAL, src, to);
                    }
                    else
                    {
                        _Convert1by1(OX.VM.OpCode.EQUAL, src, to);
                        _Convert1by1(OX.VM.OpCode.NOT, null, to);
                    }
                    ////各类!=指令
                    ////有可能有一些会特殊处理，故还保留独立判断
                    ...
                    //    _Insert1(OX.VM.OpCode.EQUAL, "", to);
                    //    return 0;
                    //}
                    //_Convert1by1(OX.VM.OpCode.INVERT, src, to);
                    //_Insert1(OX.VM.OpCode.EQUAL, "", to);
                ...
                else if (src.tokenMethod.Contains("::get_Length("))
                {
                    ...
                    _Convert1by1(OX.VM.OpCode.SIZE, src, to);
                    return 0;
                }
                ...
                {
                    throw new Exception("oxmachine cant use this call,please use  .SubString(1,2) with 2 params.");
                }
            ...
            {//翻转参数顺序
                ...
                if (havethis && calltype == 7)
                    pcount++;
                //if ((calltype == 3) || ((calltype == 7) && (callcodes[0] == OX.VM.OpCode.SYSCALL)))
                //    pcount++;
                ...
                {
                    _Insert1(OX.VM.OpCode.SWAP, "swap 2 param", to);
                }
                else if (pcount == 3)
                {
                    _InsertPush(2, "swap 0 and 2 param", to);
                    _Insert1(OX.VM.OpCode.XSWAP, "", to);
                }
                else
                {
            ...
            if (calltype == 1)
            {
                if (this.outModule.option.useNep8)
                {
                    ...
                    var c = _Convert1by1(OX.VM.OpCode.CALL_I, null, to, new byte[] { _rvcount, _pcount, 0, 0 });
                    c.needfixfunc = true;
                ...
                {
                    var c = _Convert1by1(OX.VM.OpCode.CALL, null, to, new byte[] { 5, 0 });
                    c.needfixfunc = true;
                    c.srcfunc = src.tokenMethod;
                }
                return 0;
            ...
            /*
                        else if (calltype == 3)
                        {
                            ...
                            {
                                //now oxvm use ineropMethod hash for syscall.
                            ...
                            _Convert1by1(OX.VM.OpCode.SYSCALL, null, to, outbytes);
                            return 0;
                        }
            */
            else if (calltype == 7)
            {
                for (var j = 0; j < callcodes.Length; j++)
                {
                    ...
                    if (callcodes[j] == OX.VM.OpCode.SYSCALL)
                    {
                        //if(isHex)
                        //{
                        //    throw new Exception("oxmachine OpCodeAttribute field OpData currently supports SYSCALL only with plain non-empty text (not hex)!");
                        //}
            ...
            else if (calltype == 4)
            {
                if (this.outModule.option.useNep8)
                {
                    ...
                    else
                    {
                        ...
                        _Convert1by1(OX.VM.OpCode.CALL_E, null, to, bytes);
                ...
                {
                    _Convert1by1(OX.VM.OpCode.APPCALL, null, to, callhash);
                }
            ...
            else if (calltype == 5)
            {
                ...
                _Convert1by1(OX.VM.OpCode.PACK, null, to);

                //a syscall
                {
                    var bytes = Encoding.UTF8.GetBytes("OX.Runtime.Notify");
                    byte[] outbytes = new byte[bytes.Length + 1];
                    outbytes[0] = (byte)bytes.Length;
                    Array.Copy(bytes, 0, outbytes, 1, bytes.Length);
                    //bytes.Prepend 函数在 dotnet framework 4.6 编译不过
            ...
            else if (calltype == 6)
            {
                ...
                if (this.outModule.option.useNep8)
                {
                    ...
                    //    );
                    _Convert1by1(OX.VM.OpCode.CALL_ED, null, to, new byte[] { _rvcount, _pcount });
                }
                else
                {
                    byte[] nullhash = new byte[20];
                    _Convert1by1(OX.VM.OpCode.APPCALL, null, to, nullhash);
                }
        ...
        private int _ConvertStringSwitch(ILMethod method, OpCode src, OXMethod to)
        {
        ...
        private bool TryInsertMethod(OXModule outModule, Mono.Cecil.MethodDefinition method)
        {
        ...
        private int _ConvertNewArr(ILMethod method, OpCode src, OXMethod to)
        {
            var type = src.tokenType;
            if ((type != "System.Byte") && (type != "System.SByte"))
            {
                _Convert1by1(OX.VM.OpCode.NEWARRAY, src, to);
            ...
            else // (type == "System.Byte") || (type == "System.SByte")
            {
                ...
                //we need a number
                if (code.code > OX.VM.OpCode.PUSH16)
                {
        ...
        private int _ConvertInitObj(OpCode src, OXMethod to)
        {
            var type = (src.tokenUnknown as Mono.Cecil.TypeReference).Resolve();
            _Convert1by1(OX.VM.OpCode.NOP, src, to);//空白
            _ConvertPush(type.Fields.Count, null, to);//插入个数量
            if (type.IsValueType)
            {
                _Insert1(OX.VM.OpCode.NEWSTRUCT, null, to);
            }
            else
            {
                _Insert1(OX.VM.OpCode.NEWARRAY, null, to);
            }
            ...
            //_Convert1by1(OX.VM.OpCode.DUP, null, to);
            //_Convert1by1(OX.VM.OpCode.TOALTSTACK, null, to);
            _Convert1by1(OX.VM.OpCode.DUPFROMALTSTACK, null, to);

            _InsertPush(2, "", to);//move item
            _Insert1(OX.VM.OpCode.ROLL, null, to);

            _InsertPush(2, "", to);//move value
            _Insert1(OX.VM.OpCode.ROLL, null, to);

            _Insert1(OX.VM.OpCode.SETITEM, null, to);

            ////然後要將計算棧上的第一個值，寫入第二個值對應的pos
            //_Convert1by1(OX.VM.OpCode.SWAP, null, to);//replace n to top

            ////push d
            //_Convert1by1(OX.VM.OpCode.DEPTH, null, to);

            //_Convert1by1(OX.VM.OpCode.DEC, null, to);//d 多了一位，剪掉
            //_Convert1by1(OX.VM.OpCode.SWAP, null, to);//把n拿上來
            ////push n
        ...
        private int _ConvertNewObj(OpCode src, OXMethod to)
        {
                ...
                if (m.IsConstructor && m.HasCustomAttributes)
                {
                    foreach (var attr in m.CustomAttributes)
                    {
                        if (attr.AttributeType.Name == "OpCodeAttribute")
                        {
                            ...
                            var opcode = (OX.VM.OpCode)attr.ConstructorArguments[0].Value;
                            var opdata = Helper.OpDataToBytes((string)attr.ConstructorArguments[1].Value);
                            OX.VM.OpCode v = (OX.VM.OpCode)opcode;
        ...
        private int _ConvertStfld(ILMethod method, OpCode src, OXMethod to)
        {
            ...
            _Convert1by1(OX.VM.OpCode.SWAP, null, to);//把item 拿上來

            _Convert1by1(OX.VM.OpCode.SETITEM, null, to);//修改值 //item //index //array
            return 0;
        }

        private int _ConvertLdfld(OpCode src, OXMethod to)
        {
            var field = (src.tokenUnknown as Mono.Cecil.FieldReference).Resolve();
            var type = field.DeclaringType;
            var id = type.Fields.IndexOf(field);
            ...
            _Convert1by1(OX.VM.OpCode.PICKITEM, null, to);//修改值
