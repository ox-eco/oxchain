...
namespace OX.Compiler
{
    public class Program
    {
        ...
        public static int Main(string[] args)
        {
            ...
            if (args.Length == 0)
            {
                log.Log("OX.Compiler.MSIL console app v" + Assembly.GetAssembly(typeof(Program)).GetName().Version);
                log.Log("You need a parameter to specify the DLL or the file name of the project.");
                log.Log("Examples: ");
                log.Log("  oxn mySmartContract.dll");
                log.Log("  oxn mySmartContract.csproj");
            ...
            if (file == null)
            {
                ...
                log.Log("Example:oxn abc.dll --compatible");
                return 0;
            }
        ...
        public static int Compile(string file, bool bCompatible, ILogger log = null)
        {
            ...
            switch (fileInfo.Extension.ToLowerInvariant())
            {
                ...
                default:
                    {
                        log.Log("File format not supported by oxn: " + path);
                        return -1;
                    }
            }
            ...
            try
            {
                ...
                OXModule am = conv.Convert(mod, option);
                bytes = am.Build();
